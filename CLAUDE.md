...
- Display: **Fraunces** (variable serif). The Sacred Geometry finale is a **BOLD, screen-filling, TWO-TIER closing statement** (live 2026-06-03, on top of the cinematic hero). **"Sacred geometry"** is the dominant title — TRUE Fraunces **700** at `font-variation-settings: "opsz" 48, "wght" 700`, `clamp(58px, 15vw, 232px)`, lineHeight 0.86, italic *geometry* — sitting **above** a deliberately ~4× smaller subordinate clause **"— the order beneath all things."** (`opsz 36 / wght 600`, `clamp(22px, 3.6vw, 58px)`) whose closing period is the one rust note (`#c97844`). It fills a `min-h-[100svh]` centered section. Below: a hairline rule, Stephen's verbatim words (italic `opsz 24`), and a quiet "Explore the collection →" text link. The Earth (`earth-limb.webp`, opacity 0.7) is an absolute bottom-pinned, masked background layer + a rust horizon glow + a soft radial scrim for legibility on the rose sky; **NO** rose emblem, **NO** eyebrow, **NO** mandala-ring SVG. `font-synthesis: none` makes the 700 real, never faux. See gotcha #7 for the full finale invariant list.
