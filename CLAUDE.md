...
**Colourway-set bundle** (`getColourwaySetBundle` in `paintings.ts`, surfaced as the "complete colourway set" card on PaintingDetail): buying every available colourway of one painting as a set. It reuses the SAME `bundleDiscountPercentForCount` helper, so it is honest by construction with NO `api/checkout.ts` change — the discount keys purely off basket-line count, and the "add the set" button pushes one anchor-A2 line per available colourway, so the existing count coupon fires (any painting with 3+ colourways advertises 10%, 2 → 5%; single-colourway works show no card). Decided by the `colourway-bundle-pricing` workflow: a deeper per-painting discount was rejected because it would need a colourway-COUNT mirror in `/api` — a new desync surface the recently-trimmed colourway lists would make go stale. Same single sync obligation as the collection bundle; nothing new to mirror into `/api`.
