...
- ⚠️ **KNOWN PRE-EXISTING BUG (not from Track A/B — verified on the 058ee1e deployment)**: react-helmet-async@2 is flaky-to-dead on React 19 — on direct page loads (and often SPA navs) the per-route `<title>`/meta/OG/Product JSON-LD/canonical never commit to the DOM, so every URL carries the homepage meta. The `.npmrc` legacy-peer-deps note documents the peer-range mismatch. FIX PLANNED: drop react-helmet-async and render plain `<title>`/`<meta>`/`<link>` in `Seo.tsx` — React 19 hoists document metadata natively.
