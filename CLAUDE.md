...
- **Custom size = highest option:** new `CustomSizeRequest` card under A0 ("price on application") → POSTs the NEW self-contained `api/custom-size-request.ts` → emails the estate via Resend (replyTo buyer), 200 + logs; optional env `CUSTOM_SIZE_TO_EMAIL`; needs `RESEND_API_KEY` to actually send.
...
- `CUSTOM_SIZE_TO_EMAIL` (optional): recipient for `/api/custom-size-request` (default `ESTATE_BCC_EMAIL` → info@). `api/custom-size-request.ts` added to the serverless set.
