...
**2026-06-23 — PDP world-class typography pass + "how each order arrives" + 5 small features (re-applied on top of the parallel session's live `main` — the work was first built on an older base, then rebased onto current `origin/main` and reconciled; build + `tsc -p api` + lint green; verified at 402/440/1280px).**
- **PaintingDetail redesign** (`src/pages/PaintingDetail.tsx`, from a 9-agent design workflow). Buy box → gallery wall-label: the `<h1>` stopped reusing the shared centered-section `TITLE` token (it rendered ~80px / 3-line in the ~450px column) for a LOCAL `PDP_TITLE` (Fraunces ITALIC, opsz 40, `clamp(28px,3.1vw,46px)`); added overline (replaced the outline-pill `Badge`) + "Stephen Meakin · 1966–2021" byline + a 15px wall-label spec table; breadcrumb truncated to the short collection name; price eyebrow states the format + tabular-lining figures; the ringed tier boxes → a hairline editions LIST (selected price leads, focus-visible ring); one full-width dominant CTA + ghost; `CARD` ledger geometry; captioned plate; ReassuranceRow/ProvenancePanel/grid/sticky polish. STRICTLY monochrome; `TITLE` token + `Badge` import dropped from the PDP. ⚠️ Preserved the parallel session's PDP additions (pigment note, cm+inches `sizeWithInches`, 24h-goodwill line, cross-sell rail).
- **NEW "How each order arrives" section** in the Story column (Hugo): made-to-order, hand-rolled by Point 101 (reuses `ESTATE_AUTHENTICATION.printer`), deep-red wax-seal LOGO STICKER, enclosed printed CATALOGUE/leaflet. Monochrome ledger dl.
- **Custom size = highest option:** new `CustomSizeRequest` card under A0 ("price on application") → POSTs `/api/newsletter-subscribe` with `kind:"custom-size"`; that handler is FOLDED INTO `api/newsletter-subscribe.ts` (an inline `renderCustomSizeHtml` + an early-return branch) rather than its own file — ⚠️ Vercel's Hobby plan caps a deployment at **12 Serverless Functions**, and the project was already at 12, so a standalone `api/custom-size-request.ts` made 13 and FAILED the deploy at "Deploying outputs" (build succeeded, deploy errored). Emails the estate via Resend (replyTo buyer), 200 + logs; optional env `CUSTOM_SIZE_TO_EMAIL`; needs `RESEND_API_KEY` to actually send.
- **A0 in bundles:** `Collections.tsx BUNDLE_TIER_IDS` += `"heirloom"` (per-collection / catalogue / Compose-your-own all gain A0); advertised==charged (checkout charges £1,750, margin clamp is a no-op). Editions grid `sm:grid-cols-3`→`sm:grid-cols-2 lg:grid-cols-4`.
- **Aquamarine** colourway removed from Orchis 7 (`paintings.ts`); feed 100→96 SKUs. **Mobile logo wordmark** now shows at every width (`Logo.tsx`; removed dead `wordmarkAlwaysOn`). **Red nav banner:** `.nav-bg-scrolled` near-black → deep wax-seal-red gradient (no backdrop-filter).
- ⚠️ The home wordmark "make bolder" task was SUPERSEDED by the parallel session's live `fix(home): retire heavy all-caps wordmark → refined sentence-case` — kept the live refined version (Hugo: "most aesthetic"); Welcome.tsx left untouched.
- `CUSTOM_SIZE_TO_EMAIL` (optional): recipient inbox for the custom-size enquiry (handled inside `api/newsletter-subscribe.ts`; default `ESTATE_BCC_EMAIL` → info@). NO new serverless function was added — see the 12-function cap note above.

**2026-06-18 — Multi-currency + "drop"→Edition rename + refined mastheads + real-rose cursor + smart-hide nav + fluid cross-platform type (built this session; build/lint green — only 5 PRE-EXISTING lint errors remain: memories-submit/AmbientBackdrop/EnquireModal/ui-button). Deployed at Hugo's "deploy" instruction.**

- **Customer multi-currency (display AND charge).** Catalogue stays priced in GBP pence (source of truth). New `src/lib/currency.ts` + `src/components/CurrencyProvider.tsx` (App-root) + `CurrencySelect.tsx` (header + mobile-drawer picker). Buyer picks GBP/USD/EUR/AUD/CAD; `useCurrency()` → `{code,format,formatPretty,convert,setCode}` (localStorage `tasm.currency`). Pages format via `fmt`/`fmtP` (NOT formatGBP — except PaintingDetail keeps formatGBP for the SEO/JSON-LD `fromPriceLabel` = canonical GBP). **advertised==charged in every currency:** `api/checkout.ts` reads `currency` off the POST body + charges the Stripe session in it at the SAME converted amount. ⚠️ **MIRROR (gotcha #9):** FX table + rounding live in BOTH `src/lib/currency.ts` (`CURRENCIES[*].rate`/`convertFromGbpPence`) and `api/checkout.ts` (`CURRENCY_RATES`/`convertFromGbpMinor`); both carry `CURRENCY_FX_VERSION` — change together. Rates ESTATE-SET fixed (1 GBP = 1.27 USD/1.17 EUR/1.94 AUD/1.74 CAD), rounded to whole major units. NO new env vars needed.
- **"DROP" → "EDITION" (supersedes the 2026-06-15 drops naming below).** All user-facing release language is now "edition": tiers **Open/Collector/Atelier/Heirloom Edition**; `CURRENT_DROP`→`CURRENT_EDITION` ("First Edition", id "edition-i"); `GLOBAL_DROP_NOTE`→`GLOBAL_EDITION_NOTE`; `DROP_LABEL`→`EDITION_LABEL`; webhook `LEDGER_DROP`→`LEDGER_EDITION`. Across paintings.ts + the 4 pricing mirrors + coa.ts + Auth.tsx + every page. ⚠️ The persisted ledger field names `drop_id`/`drop_label` were KEPT (legacy internal keys carrying edition values) so the dormant KV/Supabase shape stays stable — do NOT rename. The `/auth` page name consolidated "Estate Ledger" → **Estate Registry**.
- **Refined masthead system.** The giant `Fraunces 700 / opsz 48 / clamp(...,11–13vw,168–220px)` page mastheads ("too bold and unprofessional") are replaced by `src/components/PageMasthead.tsx` + `MASTHEAD_TITLE_STYLE`/`_SM` in `ui/tokens.ts`: **wght 560 (never ≥700), opsz 144, clamp(42px,7.6vw,116px), sentence case**, one italic-regular word for emphasis. From a 5-agent gallery/auction/museum study. Rolled across every content page. (SUPERSEDES the old AboutMasthead 700 recipe.)
- **Cursor = the REAL rose logo.** `.cc-flower` now uses `public/logo/cursor-rose-v1-w256.png` (the Tudor-rose keyed white-on-transparent from the source logo art), replacing the hand-drawn SVG rosette. NEW filename (immutable-cache safe).
- **Nav smart-hide + reliable backdrop.** `Nav.tsx` slides away on scroll-down / back on scroll-up (rAF, transform+opacity, NO backdrop-filter); when scrolled shows a near-solid fill (`.nav-bg-scrolled` real-CSS in global.css — the old Tailwind arbitrary `from-[#hex]/NN` gradient was fragile and left it transparent → content bled under). Inline nav + drawer switch at **3xl (1700px)** so the currency picker never overflows the bar.
- **Fluid cross-platform type ("no gaps on fullscreen").** Tailwind `3xl` 2200→**1700** (+ `4xl` 2400) so 1728/1920 screens get wide widths; `fontSize.body`/`body-sm` + `SUBTITLE` token made fluid; a 13-agent fleet made per-page fixed-px type fluid + widened measures + killed dead gaps. Verified: reminder prose 21→22.7px, columns 514→594px, section 1180→1340px at 1920.

---

**2026-06-15 — Drops model + provenance system + new wax-seal logo. ⚠️ The "drops" NAMING below is SUPERSEDED by the Edition rename above; the provenance/ledger/COA system itself is unchanged and still accurate.**

- **New logo, sitewide.** The cream line-emblem is replaced by the deep-red 3D **wax-seal Tudor rose** (`/public/logo/logo-seal-v1-*.png` — black bg keyed to feathered transparency, AI ✦ watermark cropped). Used in Nav/Footer/SiteEntrance + favicons + PWA icons (`favicon-seal-*.png`, `icon-*.png` regenerated). The white-forcing `brightness(0) invert(1)` filter was DROPPED (the seal shows in true red + a warm glow). A crisp **white line-rose** (`logo-rose-line-v1-*.png`) is the mono companion for the cursor (`.cc-flower`) where the detailed seal is too dark/small. ⚠️ `/logo` is immutable-cached — these are NEW filenames; the old `logo-emblem.svg` is no longer referenced by active code.
- **Pricing model: editions → DROPS.** Tiers renamed (ids/prices/SKUs UNCHANGED, so advertised==charged still holds): Gallery→**Open Edition** (A3, no cap, not numbered), Collector→**Collector Drop** (A2, 200/drop), Atelier→**Atelier Drop** (A1, 75/drop), Heirloom→**Heirloom Drop** (A0, 18/drop). `PrintTier.editionTotal` is now the **per-drop allocation** (null = Open). New `CURRENT_DROP` (= "Drop I") + `GLOBAL_DROP_NOTE` in paintings.ts. All "limited edition of N / never reopened / hand-numbered" scarcity copy replaced with neutral institutional language ("allocated/issued within the current drop", "numbered within its drop") across paintings.ts + the **4 pricing mirrors** (gotcha #9) + FAQ/Legal/PaintingDetail/Basket/Collections/Trade/Gift/ProvenancePanel. **Legal.tsx CCR reg-28 substance preserved** (A3 Open Edition keeps the 14-day right; Collector/Atelier/Heirloom Drops + hand-painted + framed stay made-to-order exempt). Stephen's/Polly's verbatim words untouched.
- **Estate-ledger provenance system.** On `checkout.session.completed`, `api/stripe-webhook.ts` (new block "0d", fully fail-open — never blocks the 200) issues per print line: a non-guessable **Certificate ID** `MANDALA-<ART3>-<6 Crockford-base32>` + the next **sequential print number within its drop** (Open Edition is un-numbered), written to **Vercel KV** (keys `ledger:cert:<ID>` / `ledger:seq:<artwork>:<tier>:<drop>` via INCR / `ledger:order:<sid>:<line>` for idempotency), then emails the **estate the structured Point 101 fulfilment payload** (artwork, tier, drop, print no., Certificate ID, `/auth/<id>` QR URL). Public read: **`GET /api/auth-lookup?cert=`** (public fields only — never leaks order_id). New page **`/auth` + `/auth/:certId`** (`src/pages/Auth.tsx`, "Estate Registry", QR-deep-link prefill) replaces `/verify` (now a `<Navigate>` redirect; `Verify.tsx` deleted). **`GET /api/coa?cert=&secret=&doc=certificate|label`** (ADMIN_API_KEY-gated) renders the **Certificate of Authenticity + back-of-print label PDFs** (pdf-lib) with the embedded QR + wax seal, populated entirely from the ledger. Supabase upgrade path: **`supabase/estate_ledger.sql`** (drop-in, mirrors the KV shape + `issue_print` RPC + public `auth_lookup` view). Sandbox tests: `scripts/test-provenance.mjs` (18/18) + `scripts/test-coa.mjs` (7/7). New deps: **pdf-lib, qrcode** (+`@types/qrcode`).
- ⚠️ **ACTIVATION (Hugo):** the live ledger + `/auth` need the **KV env vars** (`KV_REST_API_URL`/`KV_REST_API_TOKEN`, or `UPSTASH_REDIS_REST_*` — the SAME vars memories/webhook-dedup use). Without them, certificates aren't issued (the webhook logs a warning, the order still completes) and `/auth` falls back to the static `editions.ts` ledger. The COA endpoint needs `ADMIN_API_KEY`. To upgrade KV→Supabase later, run `supabase/estate_ledger.sql` + swap the transport in the two API functions (documented in the SQL header).
...
- **Page scene backdrops** (Collections / For You / Memories / News / Contact; About mirrors the home peacock crossfade instead): pre-blurred photos in `/img/scenes` at ONE unified recipe (resize to 1200px height → sigma-4 gaussian → webp q84; set in `37a1223`), rendered as a fixed z-0 layer under the shared scrim `linear-gradient(180deg, rgba(8,7,6,0.38) 0%, rgba(8,7,6,0.60) 42%, rgba(8,7,6,0.80) 100%)`. ⚠️ **Hugo's brightness rule (2026-06-12): a backdrop must never out-shout the cream text — "the brighter images with more going on takes too much attention… the darker ones are okay". Agent-check EVERY new or changed background image, every time: composite it under the scrim and judge against the approved-dark band (composited mean luminance ≈18–29; calibration references genesis-blur-v2 = 18.6, news-indigo-carpet-blur-v2 = 22.1). Flagged scenes get brightness/saturation baked DARKER into the asset under a NEW `-vN` filename (immutable /img cache) — never compensate with page-local scrim tweaks.** Current live set: habundia-blur-**v4**, genesis-blur-v2, born-in-the-sky-blur-v2 (the σ6 nebula everything was matched to), foryou-woodland-blur-**v3** (already dark — kept), foryou-dusk-garden-blur-**v4**, memories-plumage-blur-**v4**, news-indigo-carpet-blur-v2, contact-autumn-tree-blur-**v4**. The **v4** pass (2026-06-15, driven by a 3-judge agent panel) re-darkened the four scenes the panel flagged for bright top-bands that mean-luma alone missed: each is `modulate(brightness,saturation)` off the soft **v2** base (blur INHERITED from v2 — not re-blurred, so the nebula-match holds) + a baked top-down gradient (black α≈0.40→0 over the top 55%) so the bright sky-band/sunset/canopy drops into the dark family too — composited full-frame luma now ≈18–20, top-band ≈22–23. memories-plumage additionally got a harder desaturate + contrast-compress + slight extra soften to calm the busy peacock-eye ocelli (the issue there was busyness, not luma). woodland stayed **v3** (panel-unanimous keep). ⚠️ Laplacian-variance is NOT a valid cross-image blur metric here — it's contaminated by brightness/contrast, so darker images read as falsely sharper; judge blur visually at matched brightness, not by that number. Unblurred `*-source.jpg` siblings stay committed for regeneration. `/for-you` runs the two scenes as a whole-page-progress cross-dissolve (woodland [0,0.40,0.62]→[1,1,0], garden [0.40,0.62,1]→[0,1,1]) — per-section refs couldn't express "lower half" because the results grid starts inside the first viewport.
...
/about                       Long-form bio — CHAPTERED MONOGRAPH (redesigned 2026-06-12 to the owner's layout-PDF order beat-for-beat). One editorial signature reused throughout: ChapterHead (hairline + "Chapter {I–IX} · kicker · tag" + a ghost Bacon-style watermark year) driven from one CHAPTERS config so order is structurally enforced (the fixed I–IX ChapterRail was REMOVED 2026-06-17 — it overlapped the masthead at xl widths); Plate (family-album register — personal snapshots whole at native ratio on a warm bg-ink/[0.04] paper mount, NO crops/black-boxes/walls); Dinkus + drop caps + claim-free PlateCaption convention. Each chapter gets EITHER the ghost year OR one bespoke display moment (poster/promoted-photo/letter), never both. Order: Hero → prologue → wordless photo cluster → I Beginnings → II Bournemouth → III Wandering → IV Return & the Anegada poster (sole giant CONTAINED moment, "Everything / is connected." Fraunces 700) → V Art as ritual (sticky cymatics chart) → VI Four traditions → VII Exhibitions flowing INTO the interview photo-essay (Time Out Dubai 2011, content.ts INTERVIEW + photos 28–36) → Force India design plate → VIII TAGA (+ palestine inside) → IX Az-Zarqa & the STUDENTS LETTER (emotional heart — artefact on warm paper, roman Fraunces, sign-off landing alone via a single display slice) → Academy-close photos → "The body of work" coda → In Memoriam (TRIBUTE, untouchable) → CTA. Backdrop = home's 4-colourway peacock crossfade, breakpoints retuned so each colour turn lands on an act seam. content.ts verbatim untouched; captions claim-free (sand-mandala never claims Anegada/1995/"first"; flyer stays Fairmont-worded).
...
/auth                        Authentication — the Estate Registry. Looks a Certificate ID up in the live estate ledger (KV) via GET /api/auth-lookup, falling back to the static src/data/editions.ts. Linked from Nav/Footer/ProvenancePanel/AuthenticationCard/FAQ. (NEW 2026-06-15, drops/provenance)
/auth/:certId                Same page, Certificate ID prefilled + auto-looked-up (the COA QR code deep-links here).
/verify                      308-style SPA redirect → /auth (old links + printed certificates keep resolving; Verify.tsx deleted).
...
4. **API functions in `/api`** — `vercel.json` rewrite uses a negative lookahead to exclude `/api/*` from the SPA fallback. Don't simplify the rewrite to `/(.*)` — that breaks every serverless function. **Extended 2026-06-11** to also exclude the static asset prefixes (`assets/ img/ video/ fonts/ logo/`): existing files were always served by the filesystem first, but a MISSING asset used to fall through to the rewrite and return 200 + index.html, masking broken references (and confusing deploy checks — see the image-cache memory note). Now missing assets 404 honestly. No route may ever be created under those prefixes.
