...
- Display: **Fraunces** (variable serif). The Sacred Geometry finale is "carved": genuine `font-variation-settings: "opsz" 144, "wght" 600` (the dramatic high-contrast display cut) — NOT `font-black`/900, which the browser FAUX-bolds because only 400/600 load (this was the old "weak/muddy" finale). Size `clamp(64px,22vw,560px)`, tracking-[-0.04em], leading-[0.9]; rust period + a rust horizon-glow in the text-shadow stack. ⚠️ gotcha #7: the finale clamp is coupled to the Earth's negative `marginTop` (currently `clamp(64px,22vw,560px) * -0.40`) — edit both together.
